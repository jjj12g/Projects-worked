...
{
    // 박스의 X, Y 확장을 InSkillData.Size 값으로 사용하고, Z는 임의 값 (예: 50.f)
...
void ASkill2DBoxCollisionActor::Tick(float DeltaTime)
{
    ...
    // 1. 디버그: 현재 박스 콜리전 영역을 DrawDebugBox()로 그립니다.
    //if (CollisionBox)
    //{
    //    DrawDebugBox(
    //        GetWorld(),
    ...
    // 2. 2D 충돌 판정을 위해 자신의 위치와 2D 충돌 반경 계산
    ...
    // 3. 월드에서 ABaseCharacters(플레이어) 액터들을 검색
    ...
    for (AActor* Actor : FoundPlayers)
    {
        ...
        // 4. Z축 차이를 계산하고, 100 이상 차이나면 충돌 처리 안 함
        ...
        // 플레이어의 2D 충돌 반경은 플레이어 클래스에 정의된 CollisionRadius2D(예: 캡슐 반지름이나 별도의 값)를 사용
